...
# -*- coding: utf-8 -*-
"""
AutoEarn — 开源自动化赚/省钱助手
================================

监控加密货币/股票价格 + 商品降价，自动推送通知。
支持 Telegram Bot、Windows 桌面通知、定时调度。

功能:
  - 行情预警：BTC/ETH/股票 突破目标价自动通知
  - 价格监控：商品降价到目标价以下弹窗提醒
  - 自动调度：每 30 分钟自动检查一轮
...
CONFIG_PATH = BASE_DIR / "autoearn_config.json"
DB_PATH = BASE_DIR / "autoearn_data.csv"
LOG_PATH = BASE_DIR / "autoearn_log.txt"
...
def cmd_run() -> None:
    ...
    log.info("🚀 AutoEarn 自动化已启动 (每 30 分钟检查)")
    ...
    try:
        ...
        log.info("🛑 AutoEarn 已停止")


def cmd_daemon() -> None:
    """后台启动 (新窗口)"""
    script = Path(__file__).resolve()
    cmd = f'start "AutoEarn" python "{script}" run'
    subprocess.Popen(cmd, shell=True)
    log.info("🔄 AutoEarn 已在后台窗口启动")
...
def cmd_status() -> None:
    ...
    if _RICH:
        console.print(Panel(
            Text("🐷 AutoEarn 统计看板", style="bold cyan"),
            box=box.ROUNDED,
        ))
        ...
        # 行情预警
        if cfg.alerts:
            t2 = Table(title="📈 行情预警", box=box.SIMPLE)
        ...
        # 价格监控
        if cfg.price_items:
            t3 = Table(title="💰 价格监控", box=box.SIMPLE)
